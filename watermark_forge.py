...
# `watermarklab` (used for the VINE override) calls hf_hub_download(use_auth_token=True)
# for its public model repo - an outdated argument newer huggingface_hub rejects, and
# passing token=True without a login makes it require real auth for a public repo.
# Patch it to just not send a token at all.
...
DEFAULT_BUDGET = 0.022  # the team's proven, already-scored operating point

# --- preference-model extraction hyperparameters ---
MODEL_SIZE = 768
EXTRACT_STEPS = 400  # paper: k=50 trades quality for speed, k=500 is "high quality";
                     # 200->400 is the next step toward that ceiling (200 already beat 50 significantly)
EXTRACT_LR = 0.05
EXTRACT_MINIBATCH = 5  # sub-batch size at 768x768; 6+ spills past an 8GB GPU
TRIM_FRAC = 0.12  # trimmed mean over the 25 per-image residuals


# ===========================================================================
# 1. Preference-model watermark extraction
# ===========================================================================
...
# ===========================================================================
# 2. NLM + phase-locking-value watermark extraction
# ===========================================================================
def gauss(a, s):
...
# ===========================================================================
# 3. Template caching (extraction is the expensive step - cache per group)
# ===========================================================================
...
# pref_weight -> score, found by sweeping the blend ratio (budget fixed at 0.022):
#   1.00 (pure pref)  -> 0.4970
#   0.65               -> 0.5433 (tied with 0.50)
#   0.50 (50/50)      -> 0.5433
#   0.35              -> 0.5465  <- current best
#   0.00 (pure nlm)   -> 0.4672
...
# ===========================================================================
# 4. Proven embedding pipeline (YCbCr chroma boost + JND mask + LPIPS budget)
# ===========================================================================
def rgb2ycc(d):
    r, g, b = d[..., 0], d[..., 1], d[..., 2]
    return np.stack([
...
# ===========================================================================
# 5. Known-scheme overrides - use the real encoder where the scheme is
#    identifiable, instead of estimating a residual.
# ===========================================================================
def discover_rivagan_message(group, min_agreement=0.8):
    """Decode all 25 source images of `group` with imwatermark's RivaGAN
    detector and return the per-bit majority-vote message IF enough images
    agree (a real shared message shows strong consensus; an unrelated scheme
    decodes as near-random noise - full disagreement, no consensus). Returns
    None if the group doesn't look like RivaGAN."""
    import cv2
...
def discover_trustmark_secret(group, model_type="Q", min_agreement=0.9):
    """Decode all 25 source images of `group` with TrustMark's detector and
    return the secret string IF every (or nearly every) image agrees on the
    same secret with `present=True`. Tries model_type Q/C/B are NOT tried
    here automatically - call once per model_type since each downloads/loads
    a separate checkpoint; loop externally if needed."""
...
def discover_vine_message(group, img_size=256, min_agreement=0.9):
    """Decode all 25 source images of `group` with watermarklab's VINE
    detector (model_type 'R') and return the 100-bit majority-vote message IF
    strong consensus. Requires `pip install watermarklab` and a CUDA GPU (the
    library's embed/extract path has a device-handling bug that crashes on
    CPU-only)."""
...
# Confirmed via discover_rivagan_message('WM_2') -> 22/25 exact agreement,
# discover_trustmark_secret('WM_7', 'Q') -> 25/25 exact agreement, and
# discover_vine_message('WM_4') -> 25/25 exact agreement. The other 5 groups
# showed no consensus against RivaGAN, TrustMark Q/C/B, VINE, StegaStamp,
# InvisMark, DWT-DCT/DwtDctSvd, or Meta's VideoSeal family
# (v1.0/v0.0/PixelSeal/ChunkySeal) - left as unidentified, handled by the
# blend estimate.
GROUP_OVERRIDES = {
...
def load_override_encoder(override):
    """Load the encoder once per group (not once per image - RivaGAN's ONNX
    session, TrustMark's checkpoint, and VINE's model are all expensive to
    (re)load)."""
